...
const navItems = [
  ...
  { href: '/admin/stock', icon: Boxes, label: 'Inventori Stok' },
  { href: '/admin/inventory', icon: PackageCheck, label: 'Stok Gerobak' },
  { href: '/admin/bahan', icon: Receipt, label: 'Belanja Bahan' },
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-canvas flex text-zinc-900 antialiased">
      {/* Sidebar - Desktop */}
      <aside className="hidden lg:flex lg:flex-col lg:w-60 bg-white border-r border-zinc-200/80 fixed inset-y-0 z-40">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all ${
                  isActive
                    ? 'bg-red-50 text-brand shadow-card'
        ...
        <div className="p-3 border-t border-zinc-200/80">
          ...
          <button
            onClick={signOut}
            className="flex items-center justify-center gap-2 px-3 py-2 text-xs font-semibold text-zinc-600 hover:text-brand hover:bg-red-50 rounded-xl w-full transition-colors"
          >
